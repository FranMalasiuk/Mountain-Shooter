...
from code.Const import C_YELLOW, SCORE_POS, MENU_OPTION, C_PINK, C_WHITE
...
class Score:
    ...
    def save(self, game_mode: str, player_score: list[int]):
        pygame.mixer_music.load('./asset/MenuSong.mp3')
        pygame.mixer_music.play(-1)
        db_proxy = DBProxy('DBScore')
        name = ''
        while True:
            self.window.blit(source=self.surf, dest=self.rect)
            self.score_text(48, 'PARABÉNS, VENCEDOR!', C_YELLOW, SCORE_POS['Title'])
            if game_mode == MENU_OPTION[0]:
                score = player_score[0]
                text = 'Player 1, enter your name (4 characters)'
            if game_mode == MENU_OPTION[1]:
                score = (player_score[0] + player_score[1]) / 2
                text = 'Team, enter your name (4 characters)'
            if game_mode == MENU_OPTION[2]:
                if player_score[0] >= player_score[1]:
                    ...
                    text = 'Player2, enter your name (4 characters)'
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == K_RETURN and len(name) == 4:
                        ...
                        name += event.unicode.upper()  # Optional
